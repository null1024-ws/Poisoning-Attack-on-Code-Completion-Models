...
def obfuscate_and_detect_cycle(original_code, gpt_model, target_word, cycle_budget):
    ...
    for cycle_count in range(cycle_budget):
        ...
        found_word = detect_by_llm(last_successful_obfuscation, gpt_model, target_word)
        
        if not found_word:
            return cycle_count + 1, last_successful_obfuscation
    
    print(last_successful_obfuscation)
...
def calculate_expense(original_code, gpt_model, target_word, round_number, cycle_budget):
    ...
    for _ in range(round_number):
        print(f"Working on round {_ + 1}...")
        start_time = time.time()
        cycle_count, _ = obfuscate_and_detect_cycle(original_code, gpt_model, target_word, cycle_budget)
        end_time = time.time()
...
pattern.search(data)"""  
    ...
    cycle_counts, execution_times = calculate_expense(original_code, gpt_model, target_word, round_number, cycle_budget)
